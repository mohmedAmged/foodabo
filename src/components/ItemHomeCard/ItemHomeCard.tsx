...
import { RankedItem } from "store/UseHomeRankedItemsStore";
import Cookies from "js-cookie";
import SaleOffBadge from "components/SaleOffBadge/SaleOffBadge";

export interface CardRankedItemProps {
  className?: string;
  rankedItem: RankedItem;
}


const ItemHomeCard: FC<CardRankedItemProps> = ({
  className = "",
  rankedItem,
}) => {
      ...
      <div className="relative flex-shrink-0 ">
        ...
        <div className="absolute top-3 inset-x-3 flex justify-between">
          <div className="py-1 px-4 bg-neutral-100 dark:bg-neutral-800 rounded-full flex items-center justify-center leading-none text-xs font-medium">
            {convertNumbThousand(rankedItem?.number_of_recommendations)}{" "}
            <ArrowRightIcon className="w-5 h-5 text-yellow-600 ml-3" />
          </div>
          {rankedItem?.discount_value !== null && <SaleOffBadge desc={rankedItem?.discount_value} className="" />}
        </div>
      </div>
      <div className="pt-[1px] px-6 text-center flex flex-col items-center relative -translate-y-7">
        ...
        <span className="absolute top-2">
          <Avatar
            ...
            radius="rounded-full"
            imgUrl={rankedItem?.restaurant_logo}
            userName={rankedItem?.restaurant_name}
          />
        </span>
        <div className="mt-6">
          <h2 className={`text-base font-medium`}>
            <span className="line-clamp-1">{rankedItem?.title}</span>
          </h2>
          <span className="text-md font-semibold text-secondary-700">
            {rankedItem?.discount_value !== null ? (
              <>
                <span className="line-through text-gray-500 ml-3">
                  {rankedItem?.price_with_currency}
                </span>
                <span className="text-primary-600">
                  {rankedItem?.price_after_discount}
                </span>
              </>
            ) : (
              rankedItem?.price_with_currency
